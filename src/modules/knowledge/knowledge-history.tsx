...
const KnowledgeHistory: React.FC<Props> = ({
  docs,
  images,
  onDelete,
  onUpdate,
  ...
  function Content({ doc }: any) {
    const { dateStr, title, filePath, fullText, isImageRelation } =
      extractFile(doc);

    return (
      <>
        <TableCell
          className="font-medium text-xs text-ellipsis overflow-hidden align-middle items-center flex"
          style={{ maxWidth: "35vw" }}
        >
          {isImageRelation && (
            <span className="mr-2">
        ...
        <TableCell align="center">
          <Tooltip>
            ...
            <TooltipContent>Open {filePath}</TooltipContent>
          </Tooltip>
        </TableCell>
            ...
            <div className="flex flex-col">
              <div className="p-4 overflow-auto text-xs flex flex-1 flex-col">
                <Textarea
                  className="flex-1"
                  ref={textRef}
                  defaultValue={selectedFile?.fullText ?? ""}
                  style={{ flex: "1" }}
                />
                <div className="my-2 flex gap-1 justify-center align-middle items-center">
                  ...
                  <Input
                    className="flex-1"
                    defaultValue={selectedFile?.filePath}
                    ref={filePathRef}
                  />
                </div>
                <div className="my-2 flex gap-1 justify-center align-middle items-center">
  ...
  return (
    <>
      <DraggableComponent className="px-4 pb-2 flex-1 max165h">
        <TextContent />
        <hr />
        <ImageContent />
      </DraggableComponent>
