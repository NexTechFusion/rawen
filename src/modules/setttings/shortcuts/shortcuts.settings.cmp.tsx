...
const ShortcutSettings: React.FC<ShortcutSettingsProps> = ({ viewModel, update }) => {
    ...
    function filter(txt: string) {
        if (txt === "") {
            ...
            const filtered = shortcuts.filter((s) => s.name.toLowerCase().includes(txt.toLowerCase()));
    ...
    return (
        <div className="flex flex-1 flex-col mt-1">
                            ...
                            <div className="flex items-center">
                                <Dialog>
                                    ...
                                    <DialogContent>
                                        <DialogHeader>
                                            <DialogDescription>
                                                <div className="text-left">
                                                    <div className="mb-2">
                                                        <div>Name</div>
                                                        <Input defaultValue={shortcut.name} onBlur={(e) => updateShortcutName(shortcut.id, e.target.value)} />
                                                    </div>
                                                    <div className="mb-2 pt-2">
                                                        <div className="font-medium text-lg text-primary">Commands</div>
                                                        <hr />
                                                        <div className="bg-accent/10 p-2">
                                                            <ShortCutCommand shortcut={shortcut} update={updateCommands} isStartup={false} />
                                                        </div>
                                                    </div>
                                        ...
                                        <DialogFooter style={{ justifyContent: "center" }} className="pt-2">
                                            <DialogPrimitive.Close aria-label="Close">
                                                <Button variant="ghost">Cancel</Button>
                                                <Button type="submit" className="w-28" variant="default" onClick={saveShortcut}>Save</Button>
            ...
            <div className="flex border-t py-2">
                <Button variant="default" onClick={addShortcut}>+ Add</Button>
                <Input name='prompt' className='border-0 mx-2 flex-1' placeholder="Search..."
