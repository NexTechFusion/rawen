...
export const AppProvider = ({ children }) => {
  ...
  function checkForResultChange(newResults) {
    const currentLlmResult =
      appState?.llmResults?.find((r) => r.convoId == getConvoId())?.results ??
      [];
    const newllmResult =
      newResults?.find((r) => r.convoId == getConvoId())?.results ?? [];

    if (newllmResult.length > 0) {
      const isEq =
        JSON.stringify(currentLlmResult) == JSON.stringify(newResults);
      if (!isEq) {
